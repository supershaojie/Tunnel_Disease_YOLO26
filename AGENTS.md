...
3. **Search the repo and read the prototype before changing a model.** Search the whole package first (`ultralytics/utils/` holds most shared helpers). For this tunnel-crack project, before each model improvement inspect the corresponding prototype code and YAML under `D:\7.21yolo26改\YOLO26缝合`, then record the files read, their source and substantive differences. Report missing references honestly. Never copy the package-wide `tasks.py`, `loss.py`, or `tal.py`, import the entire module bundle, or make runtime code depend on this local reference directory. If two modules grow the same logic, consolidate into the shared utility and delete the duplicates. Avoid premature abstraction — three similar lines beat a helper nobody else calls.
