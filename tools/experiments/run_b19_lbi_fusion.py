"""Train the fixed LBI v1 from the archived b19 recipe and original COCO checkpoint."""
...
def require_runtime():
    ...
    print(f"LBI runtime audit: {actual}; ultralytics.__file__={ultralytics.__file__}", flush=True)
    if actual != expected:
...
class AuditedTrainer(DetectionTrainer):
    ...
    def _oom_retries(self, value):
        ...
        if value:
            ...
            if error is None:
                raise RuntimeError("LBI v1 forbids automatic batch reduction")
            raise error
    ...
    def get_model(self, cfg=None, weights=None, verbose=True):
        ...
        assert type(candidate.model[15]) is Concat_LBI_Fusion and candidate.model[15].out.weight.count_nonzero() == 0
        return candidate
...
def audit_training_setup(trainer):
    ...
    new = {k: p for k, p in model.named_parameters() if k.startswith("model.15.")}
    assert len(new) == 4 and all(id(p) in parameter_ids and p.requires_grad for p in new.values())
    signature = [
    ...
    optimizer = trainer.build_optimizer(
        ...
        trainer.args.weight_decay * trainer.batch_size * trainer.accumulate / trainer.args.nbs,
    )
...
def main():
    ...
    command = [
        sys.executable,
        str(Path(__file__).with_name("verify_b19_lbi_fusion.py")),
        "--baseline-root",
    ...
    print(f"LBI independent native preflight -> {audit_dir}", flush=True)
    subprocess.run([command[0], "-u", *command[1:]], check=True, cwd=ROOT)
