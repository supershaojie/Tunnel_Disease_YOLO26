...
if ! flock -n 9; then
    printf 'BCI is already running; no duplicate started: %s\n' "$PROJECT/${NAME}.lock" >&2
    exit 3
fi
if [[ "$STAGE" == train && -e "$RUN" ]]; then
    printf 'No new attempt started; preserving existing BCI run: %s\n' "$RUN" >&2
    exit 3
fi
...
run_stage() {
    case "$STAGE" in
        preflight|train)
            "$B19_PYTHON" -u "$WORK/tools/experiments/run_b19_bci_c2psa.py" \
                --baseline-root "$BASE" \
        ...
        test|diagnose|package)
            "$B19_PYTHON" -u "$WORK/tools/experiments/finish_b19_bci_c2psa.py" --stage "$STAGE"
            ;;
    esac
}
