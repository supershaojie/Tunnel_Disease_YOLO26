"""Prepare or train b19 + NDP-SPPF with the unchanged native training lifecycle."""
...
from tools.experiments import verify_b19_ndp_sppf_v1 as verify
...
def final_model_audit(trainer):
    ...
    branch = trainer.model.model[9]
    assert not branch.ndp_out.weight.count_nonzero()
    assert type(trainer.ema.ema.model[9]) is trainer.experiment.block_type
    common.assert_close_tree(branch.state_dict(), trainer.ema.ema.model[9].state_dict(), 0, 0)
...
class ObservedTrainer(AuditedTrainer):
    ...
    def optimizer_step(self):
        ...
        self.preclip = {
            ...
            for name, p in self.model.model[9].named_parameters()
            if name.startswith("ndp_")
        }
...
def preflight(config, evidence, directory, experiment=V1):
    ...
    from tools.experiments.finish_b19_ndp_sppf_v1 import fixed_subset
    ...
    report["input_gradients"] = []
    report["ema_ready_checks"] = []
    ...
    def setup(t):
        ...
        branch = t.model.model[9]
        state["names"] = [k for k, p in branch.named_parameters() if k.startswith("ndp_")]

        def observe_input(module, inputs):
            f2 = inputs[0]
            if "fixed_input" not in state:
                state["fixed_input"] = f2[:1].detach().clone()
            if f2.requires_grad:

                def record(grad):
                    report["input_gradients"].append(
                        {
                            "batch": len(report["batches"]) + 1,
                            "finite": bool(torch.isfinite(grad).all()),
                            "norm_unscaled": float(grad.float().norm() / t.scaler.get_scale()),
                        }
                    )

                f2.register_hook(record)

        handles.append(branch.register_forward_pre_hook(observe_input))
        state["initial_ema"] = t.ema.ema.model[9].ndp_out.weight.detach().clone()
        ...
        def observe_batch(module, inputs):
            ...
            if "fixed_image" not in state:
                state["fixed_image"] = batch["img"][:1].detach().float().clone()
            ...
            if "fp32_detection" not in report:
                ...
                with torch.random.fork_rng(devices=[0]):
                    ...
                    probe.model[9]._forward_pre_hooks.clear()
                    with torch.autocast("cuda", enabled=False):
                        loss, _ = probe({k: v.float() if k == "img" else v for k, v in batch.items()})
                        loss.sum().backward()
                    grads = {
                        k: float(p.grad.norm()) for k, p in probe.model[9].named_parameters() if k.startswith("ndp_")
                    }
                    assert torch.isfinite(loss).all() and grads["ndp_out.weight"] > 0
                    assert all(p.grad is not None and torch.isfinite(p.grad).all() for p in probe.model[9].parameters())
        ...
        def before_step(optimizer, args, kwargs):
            state["before"] = {k: p.detach().clone() for k, p in branch.named_parameters() if k.startswith("ndp_")}
            state["step_parameters"] = {}
            state["without_current_gradient"] = {}
            for name, p in ((k, p) for k, p in branch.named_parameters() if k.startswith("ndp_")):
    ...
    def batch_end(t):
        ...
        print("NDP preflight", json.dumps(row), flush=True)
        learned = report["completed_steps"] >= 2 and set(report["first_effective_update"]) == set(state["names"])
        ema_ready = False
        if learned and steps:
            # Readiness belongs to the stop condition: tiny early EMA updates may vanish in native FP16 checkpoints.
            with torch.no_grad(), torch.autocast("cuda", enabled=False):
                snapshot = copy.deepcopy(t.ema.ema).half().float().eval()
                quantized_nonzero = bool(snapshot.model[9].ndp_out.weight.count_nonzero())
                enabled = snapshot(state["fixed_image"])
                with verify.bypass(snapshot):
                    disabled = snapshot(state["fixed_image"])
                delta = max(
                    float((enabled[1]["one2one"][key] - disabled[1]["one2one"][key]).abs().max())
                    for key in ("boxes", "scores")
                )
                ema_ready = quantized_nonzero and delta > 0
                report["ema_ready_checks"].append(
                    {
                        "step": report["completed_steps"],
                        "quantized_po_nonzero": quantized_nonzero,
                        "one2one_max_abs_delta": delta,
                    }
                )
                del snapshot, enabled, disabled
        enough = learned and ema_ready
    ...
    try:
        ...
        for name in missing:
            ...
            if any(not item["connected"] for item in observed):
                ...
                reason = (
                    ...
                    if not trainer.model.model[9].ndp_out.weight.count_nonzero()
        ...
        assert not torch.equal(state["initial_ema"], ema.model[9].ndp_out.weight)
        ...
        assert any(row["finite"] and row["norm_unscaled"] > 0 for row in report["input_gradients"])
        with torch.no_grad(), torch.autocast("cuda", enabled=False):
            block = copy.deepcopy(trainer.model.model[9]).float().eval()
            captured = []
            handle = block.ndp_out.register_forward_hook(lambda m, ins, out: captured.append(out))
            block(state["fixed_input"].float())
            handle.remove()
            r = captured[0]
            assert torch.isfinite(r).all() and r.abs().max() > 0
            report["trained_residual"] = {
                "norm": float(r.norm()),
                "max_abs": float(r.abs().max()),
                "fixed_input_shape": list(state["fixed_input"].shape),
            }
        report["reload"] = verify.save_reload_check(ema, directory)
        report.update(
            accumulation_without_update=sum(not row["optimizer_attempt"] for row in report["batches"]),
            amp_skipped_updates=sum(row["amp_skipped"] for row in report["batches"]),
        )
    ...
    subprocess.run(
        [
            sys.executable,
            "-m",
            "tools.experiments.finish_b19_ndp_sppf_v1",
            "--checkpoint-check",
...
def resolve(options, experiment=V1):
    ...
    evidence["stage_attempt"] = os.environ.get("B19_STAGE_ATTEMPT")
...
def main(argv=None):
    ...
    parser.add_argument("--formal-receipt", type=Path, help=argparse.SUPPRESS)
    ...
    try:
        ...
        if options.stage == "preflight":
            ...
            return
        if options.formal_receipt is None:
            arguments = list(sys.argv[1:] if argv is None else argv)
            preflight_args = arguments.copy()
            preflight_args[preflight_args.index("--stage") + 1] = "preflight"
            child_attempt = execute_preflight(
                [sys.executable, "-u", str(Path(__file__).resolve()), *preflight_args], project, attempt
            )
            receipt_dir = Path((child_attempt / "audit_path.txt").read_text().strip())
            common.verify_preflight(receipt_dir)
            # A completely fresh interpreter constructs formal training only after the preflight process exits.
            subprocess.run(
                [sys.executable, "-u", str(Path(__file__).resolve()), *arguments, "--formal-receipt", str(receipt_dir)],
                cwd=ROOT,
                check=True,
                env={**os.environ, "B19_FORMAL_PARENT": str(os.getpid()), "B19_FORMAL_RECEIPT": str(receipt_dir)},
            )
            return
        receipt_dir = options.formal_receipt.resolve()
        assert os.environ.get("B19_FORMAL_PARENT") == str(os.getppid())
        assert os.environ.get("B19_FORMAL_RECEIPT") == str(receipt_dir)
        ...
        common.write_json(
            ...
            {
                "completed": True,
                "train_attempt": os.environ.get("B19_STAGE_ATTEMPT"),
                "preflight_attempt": child["evidence"].get("stage_attempt"),
