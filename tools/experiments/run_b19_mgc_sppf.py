"""Train the fixed MGC v1 from the archived b19 recipe and original COCO checkpoint."""
...
class AuditedTrainer(DetectionTrainer):
    ...
    def _oom_retries(self, value):
        ...
        if value:
            ...
            if error is None:
                raise RuntimeError("MGC v1 forbids automatic batch reduction")
            raise error
    ...
    def get_model(self, cfg=None, weights=None, verbose=True):
        ...
        assert type(candidate.model[9]) is MGC_SPPF and candidate.model[9].gap_out.weight.count_nonzero() == 0
        return candidate
...
def audit_training_setup(trainer):
    ...
    new = {
        k: p for k, p in model.named_parameters() if k.startswith(("model.9.gap_in.weight", "model.9.gap_out.weight"))
    }
...
def main():
    ...
    require_runtime()
    print("MGC runtime/source/args audit", flush=True)
    ...
    command = [
        sys.executable,
        "-u",
        str(Path(__file__).with_name("verify_b19_mgc_sppf.py")),
        "--baseline-root",
    ...
    print(f"MGC independent preflight -> {audit_dir}", flush=True)
    ...
    if args.stage == "preflight":
        return
    print("MGC preflight PASS; creating fresh formal model/optimizer/EMA/scaler/loader with seed42", flush=True)
    init_seeds(42, deterministic=True)
