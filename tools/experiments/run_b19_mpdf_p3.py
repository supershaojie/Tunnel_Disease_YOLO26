"""Prepare or train b19 + MPDF-P3 with the unchanged native training lifecycle."""
...
def final_model_audit(trainer):
    ...
    torch.save({k: v.detach().cpu() for k, v in branch.state_dict().items()}, trainer.save_dir / "initial_mpdf.pt")
...
def preflight(config, evidence, directory, experiment=V1):
    ...
    from tools.experiments.finish_b19_mpdf_p3 import fixed_subset
    ...
    def setup(t):
        ...
        state["initial_ema"] = t.ema.ema.model[15].project.weight.detach().clone()
    ...
    def batch_end(t):
        ...
        row = dict(
            ...
            project_norm=float(t.model.model[15].project.weight.norm()),
            groups=[
        ...
        print("MPDF preflight", json.dumps(row), flush=True)
        enough = report["completed_steps"] >= 2 and all(
            report["gradient_steps"].get(k, 0) > 0 for k in ("reduce.weight", "dw.weight", "project.weight")
        )
    ...
    try:
        ...
        assert all(report["gradient_steps"].get(k, 0) > 0 for k in ("reduce.weight", "dw.weight", "project.weight"))
        for h in handles:
            h.remove()
        handles.clear()
        ema = trainer.ema.ema
        assert trainer.ema.updates == report["optimizer_attempts"] >= 2
        assert not torch.equal(state["initial_ema"], ema.model[15].project.weight)
    ...
    subprocess.run(
        [
            sys.executable,
            "-m",
            "tools.experiments.finish_b19_mpdf_p3",
            "--checkpoint-check",
...
def main(argv=None):
    ...
    try:
        ...
        if options.stage == "preflight":
            ...
            common.write_json(
                attempt / "formula_cuda.json",
                dict(fp32=verify.formula_checks(device="cuda"), amp=verify.formula_checks(device="cuda", amp=True)),
            )
            init_seeds(42, deterministic=True)
