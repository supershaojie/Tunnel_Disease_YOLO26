"""Evaluate, diagnose and archive FDV v1 with distinct stage receipts and an immutable b19 comparison."""
...
from tools.experiments.run_b19_fdv_c2psa import NAME, audit_arguments, require_runtime
from ultralytics import YOLO
from ultralytics.nn.modules import C2PSA, FDV_C2PSA
...
def evaluate(weights, data, output, block_type, evidence, split="test"):
    ...
    assert type(model.model.model[10]) is block_type
...
def test_and_compare(run, data, baseline_best, evidence):
    ...
    candidate_val = evaluate(run / "weights/best.pt", data, output / "fdv_val", FDV_C2PSA, evidence, "val")
    baseline_val = evaluate(baseline_best, data, output / "b19_val", C2PSA, evidence, "val")
    candidate = evaluate(run / "weights/best.pt", data, output / "fdv_test", FDV_C2PSA, evidence)
    baseline = evaluate(baseline_best, data, output / "b19_test", C2PSA, evidence)
    rows = {
        key: {"b19": value, "FDV": candidate["metrics"][key], "delta": candidate["metrics"][key] - value}
    ...
    report = {
        ...
        "comparison": rows,
        "val_comparison": {
            k: {"b19": v, "FDV": candidate_val["metrics"][k], "delta": candidate_val["metrics"][k] - v}
    ...
    lines = [
        ...
        "",
        "| Metric | b19 | FDV | Delta |",
        "| --- | --- | --- | --- |",
    ]
    lines += [f"| {k} | {v['b19']:.9g} | {v['FDV']:.9g} | {v['delta']:+.9g} |" for k, v in rows.items()]
    lines += [
        "",
...
def archive_package(run, output, required):
    ...
    Adapted from the repaired CCA v2 archive routine; includes both best and last weights and JSON/CSV diagnostics.
    """
...
def package(run, output, evidence):
    ...
    diagnostic = json.loads((run / "baseline_comparison/diagnostics/fdv_c2psa_diagnostics.json").read_text())
    ...
    assert common.sha256(run / "baseline_comparison/diagnostics/fdv_c2psa_diagnostics.csv") == diagnostic["csv_sha256"]
    required = [
        "weights/best.pt",
        "weights/last.pt",
        "results.csv",
        ...
        "baseline_comparison/diagnostics/fdv_c2psa_diagnostics.json",
