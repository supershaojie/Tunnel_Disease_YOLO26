...
case "$STAGE" in
    preflight|train)
        ...
        python -c 'import sys, torch, ultralytics; from pathlib import Path; assert Path(ultralytics.__file__).resolve().parent == Path.cwd()/"ultralytics"; print(sys.executable, sys.version, ultralytics.__file__, ultralytics.__version__, torch.__version__, torch.version.cuda)'
        mkdir -p "$WORK/runs/detect"
        if python -u tools/experiments/run_b19_dcrstrip_v2.py \
            --baseline-root "$BASE" \
    ...
    test|diagnose|package)
        python -u tools/experiments/finish_b19_dcrstrip_v2.py --stage "$STAGE" --run "$RUN"
        ;;
    *)
