"""Evaluate, diagnose and archive SICR v1 with distinct stage receipts and an immutable b19 comparison."""
...
from tools.experiments.diagnose_b19_sicr_sppf import diagnose
from tools.experiments.run_b19_sicr_sppf import NAME, audit_arguments, require_runtime
...
def test_and_compare(run, data, baseline_best, evidence):
    ...
    evaluate(run / "weights/best.pt", data, output / "sicr_val", SICRSPPF, evidence, "val")
    candidate = evaluate(run / "weights/best.pt", data, output / "sicr_test", SICRSPPF, evidence)
    baseline = evaluate(baseline_best, data, output / "b19_test", SPPF, evidence)
    rows = {
        key: {"b19": value, "SICR": candidate["metrics"][key], "delta": candidate["metrics"][key] - value}
    ...
    report = {
        ...
        "preferred_target_reached": candidate["metrics"]["mAP50-95"] >= 0.510,
    }
    ...
    lines = [
        ...
        "",
        "| Metric | b19 | SICR | Delta |",
        "| --- | --- | --- | --- |",
    ]
    lines += [f"| {k} | {v['b19']:.9g} | {v['SICR']:.9g} | {v['delta']:+.9g} |" for k, v in rows.items()]
    lines += [
        "",
...
def package(run, output, evidence):
    ...
    diagnostic = json.loads((run / "baseline_comparison/diagnostics/sicr_sppf_diagnostics.json").read_text())
    ...
    required = [
        ...
        "baseline_comparison/diagnostics/sicr_sppf_diagnostics.json",
        "baseline_comparison/diagnostics/sicr_sppf_diagnostics.md",
    ]
    for stage in ("sicr_val", "sicr_test", "b19_test"):
