...
flock -n 9 || { echo 'DCS stage or deployment already active.' >&2; exit 3; }
...
if [[ "$STAGE" == train || "$STAGE" == preflight ]]; then
    COMMAND=("$B19_PYTHON" -u tools/experiments/run_b19_dcs_sppf_v2.py --stage "$STAGE" --baseline-root "$BASE"
    ...
    COMMAND=("$B19_PYTHON" -u tools/experiments/finish_b19_dcs_sppf_v2.py --stage "$STAGE" --run "$RUN")
fi
