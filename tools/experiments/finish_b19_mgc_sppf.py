"""Evaluate, diagnose and archive MGC v1 with distinct stage receipts and an immutable b19 comparison."""
...
from tools.experiments.diagnose_b19_mgc_sppf import diagnose
from tools.experiments.run_b19_mgc_sppf import NAME, audit_arguments, require_runtime
...
def test_and_compare(run, data, baseline_best, evidence):
    ...
    evaluate(run / "weights/best.pt", data, output / "mgc_val", MGC_SPPF, evidence, "val")
    candidate = evaluate(run / "weights/best.pt", data, output / "mgc_test", MGC_SPPF, evidence)
    baseline = evaluate(baseline_best, data, output / "b19_test", SPPF, evidence)
    rows = {
        key: {"b19": value, "MGC": candidate["metrics"][key], "delta": candidate["metrics"][key] - value}
    ...
    lines = [
        ...
        "",
        "| Metric | b19 | MGC | Delta |",
        "| --- | --- | --- | --- |",
    ]
    lines += [f"| {k} | {v['b19']:.9g} | {v['MGC']:.9g} | {v['delta']:+.9g} |" for k, v in rows.items()]
    lines += [
        "",
...
def archive_package(run, output, required):
    ...
    Adapted from the repaired CCA v2 archive routine; includes best/last and JSON/CSV diagnostics.
    """
    ...
    for folder, dirs, names in os.walk(run, followlinks=False):
        for name in dirs:
            ...
            if ".attempt." in name or path.is_symlink():
                exclusions.append(
                    {"path": path.relative_to(run).as_posix(), "reason": "transient attempt or directory symlink"}
                )
        ...
        for name in sorted(names):
            ...
            if (
                ...
                or path.is_symlink()
                or not path.is_file()
                or name in {"package_manifest.json", "package_result.json", "package_exclusions.json"}
            ):
                exclusions.append(
                    {
                        "path": path.relative_to(run).as_posix(),
                        "reason": "transient, symlink, nonregular or generated package metadata",
                    }
                )
                continue
            if path.suffix == ".pt" and path.relative_to(run).as_posix() not in {"weights/best.pt", "weights/last.pt"}:
                exclusions.append(
                    {
                        "path": path.relative_to(run).as_posix(),
                        "reason": "auxiliary checkpoint; canonical best/last retained",
                    }
                )
                continue
            files["run/" + path.relative_to(run).as_posix()] = path
    if not (run / "console.log").exists():
        exclusions.append(
            {
                "path": "console.log",
                "reason": "optional legacy console absent; attempt logs retained in provenance when available",
            }
        )
    common.write_json(run / "package_exclusions.json", exclusions)
...
def package(run, output, evidence):
    ...
    diagnostic = json.loads(
        (run / "baseline_comparison/diagnostics/mgc_sppf_diagnostics.json").read_text(encoding="utf-8")
    )
    ...
    required = [
        ...
        "baseline_comparison/diagnostics/sample_manifest.json",
        "baseline_comparison/comparison.json",
        "baseline_comparison/comparison.md",
        "baseline_comparison/diagnostics/mgc_sppf_diagnostics.json",
        "baseline_comparison/diagnostics/mgc_sppf_diagnostics.csv",
    ]
    for stage in ("mgc_val", "mgc_test", "b19_test"):
    ...
    for stage in ["test", "diagnose", "package"] if args.stage == "all" else [args.stage]:
        try:
            if stage == "test":
                ...
                require_runtime()
                diagnose(run / "weights/best.pt", data, run / "baseline_comparison/diagnostics")
            else:
