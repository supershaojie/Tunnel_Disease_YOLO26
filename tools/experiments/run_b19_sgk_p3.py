"""Resolve the recorded b19 recipe, audit one guided C3k2 extension, and run SGK in an isolated output."""
...
from ultralytics.nn.modules import C2PSA, C3k2, C3k2_SGK_P3
from ultralytics.nn.tasks import DetectionModel, load_checkpoint
from ultralytics.utils import LOGGER, YAML
from ultralytics.utils.torch_utils import autocast, init_seeds

NAME = "yolo26n_b19_sgk_p3_v1"
MODEL = ROOT / "ultralytics/cfg/models/26/yolo26n-sgk-p3-v1.yaml"
...
MODULE_CONFIG = dict(
    layer=16, guide=13, scale="n", groups=4, kernel=3, controller=16, formula="Y = cat(a, b + Po(T-b))"
)
...
def resolve_recipe(options, model=MODEL):
    """Validate the archived b19 recipe and classify every allowed SGK difference."""
    root = options.baseline_root.resolve()
    if Path(ultralytics.__file__).resolve().parent != ROOT / "ultralytics":
        raise RuntimeError(f"Ultralytics import is outside the SGK worktree: {ultralytics.__file__}")
    module_path = Path(sys.modules["ultralytics.nn.modules.sgk_p3"].__file__).resolve()
    if (
        module_path != ROOT / "ultralytics/nn/modules/sgk_p3.py"
        or Path(git("rev-parse", "--show-toplevel")).resolve() != ROOT
    ):
        raise RuntimeError("SGK module or Git root is outside the current worktree")
    ...
    if source.name.lower() in {"best.pt", "last.pt"}:
        raise ValueError("Do not initialize SGK from a b19 trained checkpoint.")
    ...
    differences = {k: {"b19": raw.get(k), "sgk": v} for k, v in effective.items() if raw.get(k) != v}
    ...
    evidence = dict(
        ...
        module_path=str(Path(sys.modules["ultralytics.nn.modules.sgk_p3"].__file__).resolve()),
        config_comparison={
            k: dict(b19=raw.get(k), sgk=effective.get(k), equal=raw.get(k) == effective.get(k))
...
def audit_weights(baseline, candidate, weights):
    ...
    for key, tensor in source.items():
        if key not in csd:
            unmatched[key] = "absent in both target-nc baseline and SGK" if key not in bsd else "unexpected missing key"
    ...
    if not new or any(not k.startswith("model.16.sgk.") for k in new):
    ...
    return dict(
        ...
        new_parameters=new,
        target_original_keys=[k for k in bsd if k.startswith("model.16.")],
...
class AuditedTrainer(DetectionTrainer):
    ...
    def _oom_retries(self, value):
        ...
        if value:
            ...
            if error is None:
                raise RuntimeError("SGK fixed-batch training forbids memory recovery retries")
            raise error
    ...
    def get_model(self, cfg=None, weights=None, verbose=True):
        ...
        self.initial_common = {
            k: v.detach().cpu().clone() for k, v in candidate.state_dict().items() if ".sgk." not in k
        }
        assert self.weight_audit["baseline_parameters"] == 2504190
        assert self.weight_audit["candidate_parameters"] == 2508786
        assert type(candidate.model[4]) is C3k2 and type(candidate.model[16]) is self.block_type
...
def audit_optimizer(trainer):
    """Check the final optimizer includes the sgk and all effective new parameters, and matches native b19 settings."""
    for key, parameter in trainer.model.named_parameters():
        if not key.startswith("model.16.sgk."):
            continue
        groups = [group for group in trainer.optimizer.param_groups for value in group["params"] if value is parameter]
        assert parameter.requires_grad and len(groups) == 1, f"Missing/frozen/duplicated SGK parameter: {key}"
        group = groups[0]
    ...
    actual["sgk_groups"] = [
        dict(
            ...
            parameters=[
                ...
                if k.startswith("model.16.sgk.") and any(p is q for q in group["params"])
            ],
        )
...
def structural_checks(directory, model=MODEL, block_type=C3k2_SGK_P3, nc=1):
    """Check only layer 16 changed, nano dimensions, parameter counts, and complete zero-sgk outputs."""
    original = YAML.load(ROOT / "ultralytics/cfg/models/26/yolo26.yaml")
    expected = copy.deepcopy(original)
    expected["head"][5][0] = [15, 13]
    expected["head"][5][2] = block_type.__name__
    expected["nc"] = nc
    ...
    assert audit["baseline_parameters"] == 2504190 and audit["added_parameters"] == 4596
    assert audit["candidate_parameters"] == 2508786
    assert candidate.yaml["scale"] == "n"
    assert [i for i, m in enumerate(candidate.model) if isinstance(m, block_type)] == [16]
    ...
    assert candidate.model[16].cv2.conv.out_channels == 64
    assert candidate.model[16].sgk.Ps.in_channels == 128
    ...
    for h, w in ((640, 640), (640, 960)):
        ...
        hook = candidate.model[16].register_forward_hook(record_shape)
        try:
            with torch.no_grad():
            ...
            hook.remove()
        assert shapes[-1] == [[1, 256, h // 8, w // 8], [1, 128, h // 16, w // 16], [1, 64, h // 8, w // 8]]
    report = dict(audit, layer16_shapes=shapes, detection_strides=[8, 16, 32], full_network_zero_sgk_equal=True)
    write_json(Path(directory) / "structural.json", report)
    return report


def save_reload_check(model, directory, block_type=C3k2_SGK_P3):
    ...
    code = "from tools.experiments.run_b19_sgk_p3 import reload_in_process; import sys; reload_in_process(sys.argv[1])"
...
def reload_in_process(path):
    ...
    with torch.no_grad(), torch.backends.mkldnn.flags(enabled=False):
        ...
        native.load_state_dict({k: v for k, v in model.state_dict().items() if ".sgk." not in k}, strict=True)
        report = dict(
            passed=True, state_exact=True, raw_exact=True, precision="FP16 snapshot reloaded in FP32", fuse={}
        )
        for name, original in (("native_control", native), ("sgk", model)):
            unfused, fused = copy.deepcopy(original), copy.deepcopy(original)
            calls = []
            handle = (
                fused.model[16].sgk.register_forward_hook(lambda *args: calls.append(True)) if name == "sgk" else None
            )
            try:
                before = unfused(x)
                ...
                if handle is not None:
                    assert calls
                    report["fused_sgk_calls"] = len(calls)
            finally:
                if handle is not None:
                    handle.remove()
...
SGK_PARAMETERS = ["Ps.weight", "DW5.weight", "Pb.weight", "Wk.weight", "Wk.bias", "Po.weight"]


def observation_report(names=SGK_PARAMETERS, max_batches=PREFLIGHT_MAX_BATCHES):
...
def preflight(config, evidence, directory, block_type=C3k2_SGK_P3, trainer_type=AuditedTrainer):
    ...
    try:
        ...
        params = dict(trainer.model.model[16].sgk.named_parameters())
        ...
        assert type(trainer.ema.ema.model[16]) is block_type
        assert trainer.ema.updates > 0 and torch.count_nonzero(trainer.ema.ema.model[16].sgk.Po.weight) > 0
...
def final_model_audit(trainer):
    ...
    sgk = trainer.model.model[16].sgk
    assert torch.count_nonzero(sgk.Po.weight) == 0
    assert all(p.requires_grad for p in sgk.parameters())
    assert type(trainer.ema.ema.model[16]) is trainer.block_type
...
def main(
    ...
    *,
    ...
    """Resolve and run the sole SGK candidate; missing server evidence never becomes a passing receipt."""
    ...
    parser.add_argument(
        "--baseline-root", type=Path, required=True, help="Original b19 project root; never the SGK worktree"
    )
    ...
    parser.add_argument(
        "--project", type=Path, help="Independent SGK output project; defaults to this worktree/runs/detect"
    )
    ...
    try:
        ...
        if options.stage == "train" and (project / options.name).exists():
            raise FileExistsError(f"SGK output already exists; no second run or overwrite: {project / options.name}")
        ...
        evidence["source_sha256"] = source_hashes(
            [
                entrypoint,
                Path(__file__).with_name("server_b19_sgk_p3_v1.sh"),
                Path(__file__).with_name("finish_b19_sgk_p3.py"),
                *source_files,
            ]
        )
        ...
        YAML.save(provenance / "sgk_effective.yaml", config)
        ...
        try:
            ...
            train_handler.close()
        print(f"SGK completed; validation-selected weights and provenance: {trainer.save_dir}")
        status.update(passed=True, stage="complete", receipt=str(receipt))
        return 0
    except BaseException as exc:
