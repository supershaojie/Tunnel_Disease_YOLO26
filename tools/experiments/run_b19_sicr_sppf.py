"""Train the fixed SICR v1 from the archived b19 recipe and original COCO checkpoint."""
...
class AuditedTrainer(DetectionTrainer):
    ...
    def _oom_retries(self, value):
        ...
        if value:
            ...
            if error is None:
                raise RuntimeError("SICR v1 forbids automatic batch reduction")
            raise error
    ...
    def get_model(self, cfg=None, weights=None, verbose=True):
        ...
        assert type(candidate.model[9]) is SICRSPPF and candidate.model[9].theta.count_nonzero() == 0
        return candidate
...
def audit_training_setup(trainer):
    ...
    new = {k: p for k, p in model.named_parameters() if k.startswith(("model.9.theta", "model.9.refine."))}
...
def main():
    ...
    audit_dir = args.project.resolve() / f"{NAME}_preflight"
    command = [
        sys.executable,
        str(Path(__file__).with_name("verify_b19_sicr_sppf.py")),
        "--baseline-root",
