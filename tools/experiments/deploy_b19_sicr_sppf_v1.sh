...
flock -n 9 || { echo 'SICR stage or deployment active; stopping.' >&2; exit 3; }
...
if [[ -e "$WORK" ]]; then
    ...
    if pgrep -af '[p]ython.*(run_b19_sicr_sppf|finish_b19_sicr_sppf)' ; then
        echo 'Existing SICR Python process; preserving checkout.' >&2; exit 3
    fi
    git -C "$WORK" status --short
else
    git -C "$BASE" worktree add --detach "$WORK" "$SHA"
fi
test -f "$WORK/tools/experiments/server_b19_sicr_sppf_v1.sh"
bash -n "$WORK/tools/experiments/server_b19_sicr_sppf_v1.sh"
test "$(git -C "$WORK" rev-parse HEAD)" = "$SHA"
