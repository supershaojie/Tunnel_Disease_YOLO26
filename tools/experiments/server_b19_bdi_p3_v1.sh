...
if ! flock -n 9; then
    printf 'BDI is already running; no duplicate started: %s\n' "$BASE/.experiment-locks/${NAME}.lock" >&2
    exit 3
fi
if [[ "$STAGE" == train && -e "$RUN" ]]; then
    printf 'No new attempt started; preserving existing BDI run: %s\n' "$RUN" >&2
    exit 3
fi
...
case "$STAGE" in
    preflight|train)
        COMMAND+=("$WORK/tools/experiments/run_b19_bdi_p3_v1.py"
            --baseline-root "$BASE"
    ...
    test|diagnose|package)
        COMMAND+=("$WORK/tools/experiments/finish_b19_bdi_p3_v1.py" --stage "$STAGE")
        ;;
esac
