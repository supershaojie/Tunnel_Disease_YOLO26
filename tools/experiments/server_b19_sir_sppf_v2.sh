#!/usr/bin/env bash
# Independent v2 worktree entry. Run inside an interactive tmux shell with remain-on-exit enabled.
set -euo pipefail
...
if ! flock -n 9; then
    printf 'A v2 command already holds %s; no duplicate started.\n' "$PROJECT/${NAME}.lock" >&2
    exit 3
fi
if [[ "$STAGE" == train && -e "$RUN" ]]; then
    printf 'Preserving existing v2 run: %s\n' "$RUN" >&2
    exit 3
fi
