#!/usr/bin/env bash
# Independent CCA v2 worktree entry. Run inside an interactive tmux shell with remain-on-exit enabled.
set -euo pipefail
...
if ! flock -n 9; then
    printf 'A CCA v2 command already holds %s; no duplicate started.\n' "$BASE/.yolo26n_b19_cca_fusion_v2.lock" >&2
    exit 3
fi
if [[ "$STAGE" == train && -e "$RUN" ]]; then
    printf 'Preserving existing CCA v2 run: %s\n' "$RUN" >&2
    exit 3
fi
...
printf '%s\n' "$$" > "$ATTEMPT/pid"
printf '%q ' "$BASH" "$WORK/tools/experiments/server_b19_cca_fusion_v2.sh" "$STAGE" > "$ATTEMPT/command.txt"
...
run_stage() {
    case "$STAGE" in
        preflight|train)
            "$B19_PYTHON" -u "$WORK/tools/experiments/run_b19_cca_fusion_v2.py" \
                --baseline-root "$BASE" \
            ...
            ;;
        test)
            "$B19_PYTHON" -u "$WORK/tools/experiments/finish_b19_cca_fusion_v2.py" --stage test --run "$RUN" || return $?
            local baseline_best="$BASE/runs/detect/b19_y26n_diverse5x_e200_i640_b32_musgd_b8b9hybrid_s42/weights/best.pt"
            test -f "$baseline_best" || { printf 'Required b19 comparison weight missing: %s\n' "$baseline_best" >&2; return 2; }
            "$B19_PYTHON" -u "$WORK/tools/experiments/finish_b19_cca_fusion_v2.py" --stage test --run "$RUN" --baseline-best "$baseline_best"
            ;;
        diagnose|package)
            "$B19_PYTHON" -u "$WORK/tools/experiments/finish_b19_cca_fusion_v2.py" --stage "$STAGE" --run "$RUN"
            ;;
    esac
}
set +e
