...
if [[ "$STAGE" == train || "$STAGE" == preflight ]]; then
    COMMAND=("$B19_PYTHON" -u tools/experiments/run_b19_dcs_sppf.py --stage "$STAGE" --baseline-root "$BASE"
    ...
    COMMAND=("$B19_PYTHON" -u tools/experiments/finish_b19_dcs_sppf.py --stage "$STAGE" --run "$RUN")
fi
