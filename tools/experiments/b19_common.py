"""B19 evidence and equality audits selectively reused from NDP 90e32df.
...
MODEL = ROOT / "ultralytics/cfg/models/26/yolo26n-sicr-sppf-v1.yaml"
...
def resolve_recipe(options, model=MODEL):
    ...
    import ultralytics.nn.modules.sicr_sppf as module

    assert Path(module.__file__).resolve() == ROOT / "ultralytics/nn/modules/sicr_sppf.py"
    ...
    evidence = {
        ...
        "module_path": str(ROOT / "ultralytics/nn/modules/sicr_sppf.py"),
        "python": sys.version,
...
def audit_weights(baseline, candidate, weights, new_prefix=("model.9.refine.", "model.9.theta")):
...
def source_hashes(extra=()):
    ...
    paths = (
        ...
        + sorted((ROOT / "tests").glob("test_sicr_sppf_v1*.py"))
        + list(map(Path, extra))
    )
