...
MODEL = ROOT / "ultralytics/cfg/models/26/yolo26n-bdi-p3-v1.yaml"
...
def resolve_recipe(options, model=MODEL):
    ...
    import ultralytics.nn.modules.bdi_p3 as module

    assert Path(module.__file__).resolve() == ROOT / "ultralytics/nn/modules/bdi_p3.py"
    ...
    evidence = {
        ...
        "module_path": str(ROOT / "ultralytics/nn/modules/bdi_p3.py"),
        "python": sys.version,
...
def audit_weights(baseline, candidate, weights, new_prefix="model.15."):
...
def source_hashes(extra=()):
    ...
    paths = (
        ...
        + sorted((ROOT / "tests").glob("test_bdi_p3_v1*.py"))
        + list(map(Path, extra))
    )
