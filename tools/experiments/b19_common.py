"""B19 evidence and equality audits reused from SICR d6e9009 (originally NDP 90e32df).
...
MODEL = ROOT / "ultralytics/cfg/models/26/yolo26n-fdv-c2psa-v1.yaml"
...
def resolve_recipe(options, model=MODEL):
    ...
    import ultralytics.nn.modules.fdv_c2psa as module

    assert Path(module.__file__).resolve() == ROOT / "ultralytics/nn/modules/fdv_c2psa.py"
    ...
    evidence = {
        ...
        "module_path": str(ROOT / "ultralytics/nn/modules/fdv_c2psa.py"),
        "python": sys.version,
...
def audit_weights(baseline, candidate, weights, new_prefix="model.10.m.0.attn.theta_c"):
    ...
    return {
        ...
        "rng_strategy": "native modules constructed once; only zero constants added; no additional RNG",
...
def source_hashes(extra=()):
    ...
    paths = (
        ...
        + sorted((ROOT / "tests").glob("test_fdv_c2psa*.py"))
        + list(map(Path, extra))
    )
