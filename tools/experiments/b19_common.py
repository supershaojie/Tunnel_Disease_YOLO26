...
MODEL = ROOT / "ultralytics/cfg/models/26/yolo26n-ndp-sppf-v1.yaml"
...
def resolve_recipe(options, model=MODEL):
    ...
    import ultralytics.nn.modules.ndp_sppf as module

    assert Path(module.__file__).resolve() == ROOT / "ultralytics/nn/modules/ndp_sppf.py"
    ...
    evidence = {
        ...
        "module_path": str(ROOT / "ultralytics/nn/modules/ndp_sppf.py"),
        "python": sys.version,
...
def audit_weights(baseline, candidate, weights, new_prefix="model.9.ndp_"):
...
def source_hashes(extra=()):
    ...
    paths = (
        ...
        + sorted((ROOT / "tests").glob("test_ndp_sppf_v1*.py"))
        + list(map(Path, extra))
    )
