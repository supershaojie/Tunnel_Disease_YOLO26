...
MODEL = ROOT / "ultralytics/cfg/models/26/yolo26n-mpdf-p3-v1.yaml"
...
def audit_weights(baseline, candidate, weights, new_prefix, layer):
...
def source_hashes(extra=()):
    ...
    paths = (
        ...
        + sorted((ROOT / "tests").glob("test_mpdf_p3*.py"))
        + list(map(Path, extra))
    )
