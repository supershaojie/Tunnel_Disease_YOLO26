"""Audited b19 infrastructure adapted from the fixed experiment runner at 6f5f1e2."""
...
def audit_weights(baseline, candidate, weights, new_prefix="model.23.", layer=23):
...
class AuditedTrainer(DetectionTrainer):
    ...
    block_type = DSDDetect
    layer = 23
    new_marker = "reg_adapter."
    new_parameters = 3488
...
def audit_optimizer(trainer):
    """Check the final optimizer includes the adapters and all effective new parameters, and matches native b19 settings."""
    ...
    if missing:
        raise AssertionError(f"Adapter parameters missing/frozen in final optimizer: {missing}")
...
def reload_in_process(path, block_type=DSDDetect, layer=23, new_marker="reg_adapter.", new_parameters=3488):
...
def save_reload_check(model, directory, block_type=DSDDetect, layer=23, new_marker="reg_adapter.", new_parameters=3488):
    ...
    code = """
from tools.experiments.b19_common import reload_in_process
...
def preflight(config, evidence, directory, block_type=DSDDetect, trainer_type=AuditedTrainer, batch_check=None):
    ...
    *,
    ...
    try:
        ...
        evidence["source_sha256"] = source_hashes(
            [
                entrypoint,
                Path(__file__).with_name("server_b19_dsd_head_v1.sh"),
                Path(__file__).with_name("finish_b19_dsd_head.py"),
                *source_files,
            ]
        )
