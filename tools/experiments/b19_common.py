...
MODEL = ROOT / "ultralytics/cfg/models/26/yolo26n-bci-c2psa-v1.yaml"
...
def resolve_recipe(options, model=MODEL):
    ...
    import ultralytics.nn.modules.bci_c2psa as module

    assert Path(module.__file__).resolve() == ROOT / "ultralytics/nn/modules/bci_c2psa.py"
    ...
    evidence = {
        ...
        "module_path": str(ROOT / "ultralytics/nn/modules/bci_c2psa.py"),
        "python": sys.version,
...
def dataset_manifest(data):
    ...
    for split, expected_count in REFERENCE["dataset_counts"].items():
        ...
        manifest[split] = {
            "images": len(images),
            "targets": targets,
            "image_label_content_sha256": hashlib.sha256(json.dumps(entries).encode()).hexdigest(),
        }
    return manifest


def audit_weights(baseline, candidate, weights, new_prefix="model.10.bci."):
...
def audit_optimizer(trainer):
    ...
    assert len(ids) == len(set(ids)), "Duplicated optimizer membership"
    missing = [
        k
...
def source_hashes(extra=()):
    ...
    paths = (
        ...
        + sorted((ROOT / "tests").glob("test_bci_c2psa*.py"))
        + list(map(Path, extra))
    )
...
def record_process():
    """Record every actual Python process inside its enclosing immutable stage attempt."""
    from datetime import datetime, timezone

    attempt = os.environ.get("B19_STAGE_ATTEMPT")
    if attempt:
        folder = Path(attempt)
        write_json(
            folder / f"process_{os.getpid()}.json",
            {
                "pid": os.getpid(),
        ...
        (folder / "python.pid").write_text(str(os.getpid()) + "\n")
