"""B19 evidence/equality utilities ported from FDV d0b9d6f, adapted only for LBI identity and four new tensors."""
...
MODEL = ROOT / "ultralytics/cfg/models/26/yolo26n-lbi-fusion-v1.yaml"
...
def resolve_recipe(options, model=MODEL):
    ...
    import ultralytics.nn.modules.lbi_fusion as module

    assert Path(module.__file__).resolve() == ROOT / "ultralytics/nn/modules/lbi_fusion.py"
    print(f"LBI source/args audit: HEAD={git('rev-parse', 'HEAD')} python={sys.executable}", flush=True)
    print(f"ultralytics.__file__={ultralytics.__file__}; module={module.__file__}", flush=True)
    ...
    if results.exists():
        assert sha256(results) == "44795068d0ebc687c29ff7b7b8cbf337469546b2d4dd515cb0616c270bdf9507"
    ...
    evidence = {
        ...
        "field_comparison": {
            k: {"b19": raw.get(k), "effective": effective.get(k), "equal": raw.get(k) == effective.get(k)}
            for k in set(raw) | set(effective)
        },
        ...
        "module_path": str(ROOT / "ultralytics/nn/modules/lbi_fusion.py"),
        "python": sys.version,
    ...
    expected_manifest = json.loads(Path(__file__).with_name("b19_dataset_manifest.json").read_text())
...
def dataset_manifest(data):
    ...
    for split, expected_count in REFERENCE["dataset_counts"].items():
        ...
        entries = []
        targets = 0
        print(f"LBI dataset audit: {split}, {len(images)} images", flush=True)
        for index, image in enumerate(images, 1):
            ...
            if index % 512 == 0 or index == len(images):
                print(f"LBI dataset audit: {split} {index}/{len(images)}", flush=True)
...
def audit_weights(baseline, candidate, weights):
    ...
    expected_new = {
        "model.15.proj_l.weight": [16, 128, 1, 1],
        "model.15.proj_s.weight": [16, 128, 1, 1],
        "model.15.dw.weight": [16, 1, 3, 3],
        "model.15.out.weight": [128, 16, 1, 1],
    }
    if new_state != expected_new or new != expected_new:
    ...
    return {
        "common_keys": len(bsd),
        "exact_matches": len(bsd) - len(changed),
        "common_elements": sum(v.numel() for v in bsd.values()),
        "checkpoint_tensors": len(source),
        "matched_tensors": len(loaded),
        "layer10_shared_loaded": all(k in loaded for k in bsd if k.startswith("model.10.")),
        "shared_coverage": 1.0,
        "checkpoint_tensor_coverage": len(loaded) / len(bsd),
        "loaded_keys": loaded,
        ...
        "rng_strategy": "fork_rng(devices=[]) isolates only the four new Conv2d initializations",
...
def launcher_evidence(options, raw):
    ...
    if path is None or not path.is_file():
        raise FileNotFoundError(f"Original expanded launcher required: {path}")
    import shlex
...
def source_hashes(extra=()):
    ...
    paths = (
        ...
        + sorted((ROOT / "tests").glob("test_lbi_fusion*.py"))
        + list(map(Path, extra))
    )
...
def require_clean_source():
    ...
    changed_core = git("diff", "--name-only", REFERENCE["source_commit"], "HEAD", "--", *core)
    if changed_core:
        raise RuntimeError(f"Native b19 implementation changed: {changed_core}")
