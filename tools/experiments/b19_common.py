...
MODEL = ROOT / "ultralytics/cfg/models/26/yolo26n-mgc-sppf-v1.yaml"
...
def resolve_recipe(options, model=MODEL):
    ...
    import ultralytics.nn.modules.mgc_sppf as module

    assert Path(module.__file__).resolve() == ROOT / "ultralytics/nn/modules/mgc_sppf.py"
    ...
    evidence = {
        ...
        "module_path": str(ROOT / "ultralytics/nn/modules/mgc_sppf.py"),
        "python": sys.version,
...
def audit_weights(baseline, candidate, weights, new_prefix=("model.9.gap_in.weight", "model.9.gap_out.weight")):
    ...
    return {
        "common_keys": len(bsd),
        "exact_matches": len(bsd),
        "common_elements": sum(v.numel() for v in bsd.values()),
        "checkpoint_tensors": len(source),
        "matched_tensors": len(loaded),
        "shared_exact_match": len(bsd),
        "shared_shape_mismatch": [],
        "checkpoint_shape_mismatch_count": len(unmatched),
        "load_coverage_of_shape_compatible_source": len(loaded) / (len(source) - len(unmatched)) if source else None,
        "source_tensor_coverage": len(loaded) / len(source) if source else None,
        "layer9_cv1_loaded": all(k in loaded for k in bsd if k.startswith("model.9.cv1.")),
        "layer9_cv2_loaded": all(k in loaded for k in bsd if k.startswith("model.9.cv2.")),
        "loaded_keys": loaded,
        ...
        "rng_strategy": "native modules constructed once; only added CPU Conv2d initialization uses fork_rng(devices=[])",
...
def source_hashes(extra=()):
    ...
    paths = (
        ...
        + sorted((ROOT / "tests").glob("test_mgc_sppf_v1*.py"))
        + list(map(Path, extra))
    )
