"""Prepare or train b19 + BDI-P3 with the unchanged native training lifecycle."""
...
from tools.experiments import verify_b19_bdi_p3_v1 as verify
...
def final_model_audit(trainer):
    ...
    branch = trainer.model.model[15]
    assert not branch.po.weight.count_nonzero()
    assert type(trainer.ema.ema.model[15]) is trainer.experiment.block_type
    common.assert_close_tree(branch.state_dict(), trainer.ema.ema.model[15].state_dict(), 0, 0)
...
class ObservedTrainer(AuditedTrainer):
    ...
    def optimizer_step(self):
        ...
        self.preclip = {
            ...
            for name, p in self.model.model[15].named_parameters()
        }
...
def preflight(config, evidence, directory, experiment=V1):
    ...
    from tools.experiments.finish_b19_bdi_p3_v1 import fixed_subset
    ...
    def setup(t):
        ...
        branch = t.model.model[15]
        state["names"] = list(dict(branch.named_parameters()))

        def observe_p2(module, inputs):
            f2 = inputs[0][2]
            if "fixed_p2" not in state:
                state["fixed_p2"] = f2[:1].detach().clone()
            if f2.requires_grad:

                def record(grad):
                    report["p2_gradients"].append(
                        {
        ...
        handles.append(branch.register_forward_pre_hook(observe_p2))
        state["initial_ema"] = t.ema.ema.model[15].po.weight.detach().clone()
        ...
        def observe_batch(module, inputs):
            ...
            if "fp32_detection" not in report:
                ...
                with torch.random.fork_rng(devices=[0]):
                    ...
                    probe.model[15]._forward_pre_hooks.clear()
                    with torch.autocast("cuda", enabled=False):
                        loss, _ = probe({k: v.float() if k == "img" else v for k, v in batch.items()})
                        loss.sum().backward()
                    grads = {k: float(p.grad.norm()) for k, p in probe.model[15].named_parameters()}
                    assert torch.isfinite(loss).all() and grads["po.weight"] > 0
                    assert all(
                        p.grad is not None and torch.isfinite(p.grad).all() for p in probe.model[15].parameters()
                    )
    ...
    def batch_end(t):
        ...
        print("BDI preflight", json.dumps(row), flush=True)
        ...
        if learned and steps:
            ...
            with torch.no_grad(), torch.autocast("cuda", enabled=False):
                ...
                quantized_nonzero = bool(snapshot.model[15].po.weight.count_nonzero())
    ...
    try:
        ...
        for name in missing:
            ...
            if any(not item["connected"] for item in observed):
                ...
                reason = (
                    ...
                    if not trainer.model.model[15].po.weight.count_nonzero()
        ...
        assert not torch.equal(state["initial_ema"], ema.model[15].po.weight)
        ...
        assert any(row["finite"] and row["norm_unscaled"] > 0 for row in report["p2_gradients"])
        with torch.no_grad(), torch.autocast("cuda", enabled=False):
            r = trainer.model.model[15].details(state["fixed_p2"].float())[-1]
            assert torch.isfinite(r).all() and r.abs().max() > 0
            report["trained_residual"] = {
                "norm": float(r.norm()),
                "max_abs": float(r.abs().max()),
                "fixed_input_shape": list(state["fixed_p2"].shape),
            }
    ...
    subprocess.run(
        [
            sys.executable,
            "-m",
            "tools.experiments.finish_b19_bdi_p3_v1",
            "--checkpoint-check",
