"""Independent FP32 val/test, fixed-val BDI diagnosis and verifiable result packaging."""
...
from tools.experiments.verify_b19_bdi_p3_v1 import bypass
...
def evaluate(weight, data, output, split, device=0, batch=32, workers=8, experiment=V1, threshold=None, baseline=False):
    ...
    calls = {"bdi": 0, "one2one": 0}

    def observe_bdi(module, inputs, output):
        assert output.dtype == torch.float32
        calls["bdi"] += 1

    def observe_head(module, inputs, output):
        assert module.end2end and output[1]["one2one"]["scores"].numel() > 0
        assert baseline or calls["bdi"] > calls["one2one"]
        calls["one2one"] += 1

    handles = [] if baseline else [model.model.model[15].register_forward_hook(observe_bdi)]
...
def diagnose(run, data, output, experiment=V1, device="cuda:0"):
    """Measure U/D/V3/R and matched detection changes on exactly 16 validation images."""
    ...
    for path in images:
        ...
        def observe(module, inputs, result, stats=stats):
            _, l, f2 = inputs[0]
            u, d, v3, r = module.details(f2)
            values = {
                k: {
                    "norm": float(v.norm()),
                    "finite_fraction": float(torch.isfinite(v).float().mean()),
                    "shape": list(v.shape),
                }
                for k, v in zip(("U", "D", "V3", "R"), (u, d, v3, r))
            }
            values.update(
                D_over_U=float(d.norm() / (u.norm() + eps)), R_over_L=float(r.norm() / (l.norm() + eps)), epsilon=eps
            )
            assert all(torch.isfinite(v).all() for v in (u, d, v3, r))
            stats.append(values)
        ...
        for key in ("off", "on"):
            ...
            cv2.putText(panel, f"BDI {key}; conf=0.25", (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
            panels.append(panel)
    ...
    result = {
        ...
        "real_training_preflight_gradients": {"steps": preflight["gradient_steps"], "p2": preflight["p2_gradients"]},
        "weight_sha256": common.sha256(run / "weights/best.pt"),
        "commit": common.git("rev-parse", "HEAD"),
        "source_sha256": common.source_hashes(),
        "data_sha256": common.sha256(data),
        "backend": common.computation_conditions(),
        "interpretation": "Same trained weights, residual on/off only; not a retrained ablation. D is a signed band-pass feature, not crack probability. No evaluation gradients are claimed.",
    }
...
def package(run, data, experiment=V1):
    ...
    common.write_json(
        staging / "sources.json",
        {
            ...
            "data": str(data),
            "data_sha256": common.sha256(data),
            "reference_files": json.loads((ROOT / "tools/experiments/bdi_sources.json").read_text(encoding="utf-8")),
            "references": ["https://arxiv.org/html/2412.10116v2", "https://proceedings.mlr.press/v97/zhang19a.html"],
