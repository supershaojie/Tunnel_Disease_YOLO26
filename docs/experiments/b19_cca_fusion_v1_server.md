...
WORK=/root/autodl-tmp/projects/Tunnel_Disease_YOLO26_CCA_Fusion_v1
BRANCH=codex/exp-yolo26n-b19-cca-fusion-v1
[[ "$SHA" =~ ^[0-9a-f]{40}$ ]]
...
test -f "$WORK/tools/experiments/server_b19_cca_fusion_v1.sh"
bash -n "$WORK/tools/experiments/server_b19_cca_fusion_v1.sh"
test "$(git -C "$WORK" rev-parse HEAD)" = "$SHA"
printf '部署完成: %s\n%s\n' "$WORK" "$SHA"
)
```
...
SESSION=y26_cca_v1_train
test "$(git -C "$WORK" rev-parse HEAD)" = "$SHA"
test ! -e "$WORK/runs/detect/yolo26n_b19_cca_fusion_v1"
if tmux has-session -t "$SESSION" 2>/dev/null; then echo 'tmux会话已存在，未重复启动'; exit 3; fi
tmux new-session -d -s "$SESSION"
tmux set-option -t "$SESSION" remain-on-exit on
tmux send-keys -t "$SESSION" "cd '$WORK' && B19_PYTHON='$B19_PYTHON' bash tools/experiments/server_b19_cca_fusion_v1.sh train; rc=\$?; printf '\\ntrain shell exit=%s\\n' \"\$rc\"; exit \$rc" C-m
...
B19_PYTHON=/root/miniconda3/bin/python
cd "$WORK"
B19_PYTHON="$B19_PYTHON" bash tools/experiments/server_b19_cca_fusion_v1.sh preflight
)
```
...
RUN=yolo26n_b19_cca_fusion_v1
tmux list-panes -t y26_cca_v1_train -F '#{session_name} dead=#{pane_dead} exit=#{pane_dead_status} pid=#{pane_pid}'
tmux capture-pane -pt y26_cca_v1_train -S -35
...
WORK=/root/autodl-tmp/projects/Tunnel_Disease_YOLO26_CCA_Fusion_v1
B19_PYTHON=/root/miniconda3/bin/python
RUN="$WORK/runs/detect/yolo26n_b19_cca_fusion_v1"
test "$(git -C "$WORK" rev-parse HEAD)" = "$SHA"
ATTEMPT="$(cat "${RUN}_train.current_attempt")"
test "$(cat "$ATTEMPT/exit_status")" = 0
cd "$WORK"
for STAGE in test diagnose package; do
  B19_PYTHON="$B19_PYTHON" bash tools/experiments/server_b19_cca_fusion_v1.sh "$STAGE"
done
...
b19 best存在时经相同入口复测并验证固定权重摘要；不存在则明确记录严格对照缺失。
...
真实路径由 `package_result.json` 给出，不猜文件名。归档内有best、配置、日志、指标/预测/图、预检更新证据、
源码tar与补丁、依赖版本、attempt信息和每文件大小/SHA256；不包含原始数据、Conda、.git或其他实验。
